  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        console.log('[AUTH] Trying to login with email:', email);
        ...
        if (!user || !user.password) {
          console.log('[AUTH] Login failed: No user or no password');
          return null;
        }
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.accessToken = (user as any).accessToken || token.sub;
      }
      return token;
    },
